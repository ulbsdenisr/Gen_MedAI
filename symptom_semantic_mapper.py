...
class SemanticMapper:
    ...
    score_threshold: float = 0.62   # putin mai sigur decat 0.60
    margin: float = 0.06            # putin mai permisiv decat 0.08
    topn: int = 2
    ...
    def load(self):
        ...
        if not vocab_path.exists() or not index_path.exists() or not model_path.exists():
            raise FileNotFoundError("Missing semantic mapper files. Run build_semantic_mapper.py first.")
    ...
    def _pick(self, scores: np.ndarray, ids: np.ndarray) -> Tuple[str, float]:
        """
        scores: shape (topn,), cosine/IP scores
        ids:    shape (topn,), vocab ids
        """
        best_id = int(ids[0])
        best_score = float(scores[0])

        # daca nu exista match valid
        if best_id < 0:
            return "", best_score
        ...
        # margin test (ambiguitate intre top1 si top2)
    ...
    def map_one(self, phrase: str) -> Tuple[str, float]:
        ...
        emb = self._model.encode([p], normalize_embeddings=True)
        ...
        if not mapped:
            return p, float(scores[0][0])  # fallback: original
        return mapped, score

    def map_list(self, phrases: List[str]) -> List[str]:
        """
        Varianta rapida: encode in batch + search in batch.
        Pastreaza fallback-ul: daca nu mapeaza clar, returneaza originalul curatat.
        """
        self._ensure_loaded()
        ...
        emb = self._model.encode(cleaned, normalize_embeddings=True, show_progress_bar=False)
        emb = np.asarray(emb, dtype="float32")
